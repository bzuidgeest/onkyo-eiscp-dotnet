...
namespace onkyo
{
    public static class ISCPDocumentationGenerator
    {
        ...
        //CMND(BD via RIHD)
        //CMND(TV via RIHD)
        private static string[] worksheetsToParse = { "CMND(MAIN)", "CMND(ZONE2)", "CMND(ZONE3)", "CMND(ZONE4)", "CMND(NET USB)", "CMND(PORT)", "CMND(via RI)", "CMND(BD via RIHD", "CMND(TV via RIHD)" };
        ...
        public static ISCPDocumentation Parse(string file)
        {
                                ...
                                && Regex.IsMatch(worksheet.Cells[row, column].Value.ToString(), "\"(?<command>.*)\" - (?<description>.*)") == true)
                            {
                                    ...
                                    && Regex.IsMatch(worksheet.Cells[row, column].Value.ToString(), "\"(?<command>.*)\" - (?<description>.*)") == false)
                                {
                                    ...
                                    if (worksheet.Cells[row, column].Value.ToString().StartsWith("*") == false)
                                    {
                                        ...
                                        commandDocumentation.Values.Add(new ISCPCommandValueDocumentation()
                                        {
                                            Name = worksheet.Cells[row, column].Value.ToString().Trim('”').Trim('"'),
            ...
            List<string> models = documentation.Commands.Single(x => x.Name == "DIF"
                ...
                .Values.Single(x => x.Name == "02").SupportedDevices.ToList();
            ...
            documentation.Commands.Single(x => x.Name == "DIF"
                && x.Description == "Display Mode Command")
                .Values.Single(x => x.Name == "02").SupportedDevices = models.ToArray();
